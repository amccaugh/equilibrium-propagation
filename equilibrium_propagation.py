...
def intialize_weight_matrix(layer_sizes, seed = None):
    ...
    W_mask += W_mask.T
    W = torch.from_numpy(W).float().to(device).clone() # Convert to float Tensor
    W_mask = torch.from_numpy(W_mask).float().to(device).clone() # .byte() is the closest thing to a boolean tensor pytorch has
...
def random_initial_state(batch_size = 7):
    s = torch.rand([batch_size, num_neurons], requires_grad = True)
    return s
...
def E(s, W):
    ...
    rho_s = rho(s)
    term2 = -0.5 * torch.sum(torch.matmul(rho_s.unsqueeze(2), rho_s.unsqueeze(1)) * W, dim = [1,2])
...
def evolve_to_equilbrium(s, W, d, beta, eps, total_tau,
    ...
    for n in range(num_steps):
        s = step(s, W, eps = eps, beta = beta, d = d)
        if state_list is not None: states.append(s.detach().numpy().copy())
        if energy_list is not None: energies.append(F(s, W, beta, d).detach().numpy().copy())
    return s
...
#    d = np.matmul(T,s[:,iy])
    ...
    d[:,iy] = d_values
    d.detach_().requires_grad_(False)
    return d

def generate_dummy_target(s):
    d = torch.zeros(s.shape)
    return d
...
for n in range(10):
    ...
    s = evolve_to_equilbrium(s = s, W = W, d = d, beta = 0, eps = eps, total_tau = total_tau)
#                             state_list = states, energy_list = energies)
    s_free_phase = s.clone().detach().requires_grad_(False)
    
    s = evolve_to_equilbrium(s = s, W = W, d = d, beta = 1, eps = eps, total_tau = total_tau)
#                             state_list = states, energy_list = energies)
    s_clamped_phase = s.clone().detach().requires_grad_(False)
