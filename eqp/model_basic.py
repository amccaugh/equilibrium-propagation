...
class EQP_Network(object):
    ...
    def initialize_weight_matrix(self, layer_sizes, seed = None, kind = 'layered', symmetric = True,
        ...
        if kind == 'layered':
            ...
            for n in range(len(self.layer_sizes)-1):
                ...
                # create masks to view weight matrix between two layers
                conn = np.zeros((self.num_neurons,self.num_neurons))
                conn[i:i+di, j:j+dj] = 1
                conn[j:j+dj, i:i+di] = 1
                conn = np.tril(conn)
                conn /= np.sqrt(np.count_nonzero(conn))
    ...
    def _calculate_weight_update(self, beta, s_free_phase, s_clamped_phase):
        ...
        dW *= self.W_mask
        adjustments = []
        for m in self.interlayer_connections:
            # (1/torch.sqrt(torch.norm(m,p=1)))*
            adjustment = torch.norm(torch.mean(dW,dim=0)*m)
            adjustments.append(adjustment)
        return dW, adjustments
    ...
    def train_batch(self, x_data, y_target, beta, learning_rate):
        ...
        # Update weights
        dW, adj = self._calculate_weight_update(beta, s_free_phase, s_clamped_phase)
        self.W += torch.mean(dW, dim = 0)*learning_rate
        # return s,W
        return [a.item() for a in adj]
